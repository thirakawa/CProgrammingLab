...
export default function UsersPage() {
  ...
  const load = async () => {
    try {
      ...
      setUsers(all.filter(u => u.role === 'teacher'))
    } catch (e) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      await apiCreateUser({ ...newUser, role: 'teacher' })
      setNewUser({ username: '', password: '' })
      flash('教員アカウントを作成しました')
      load()
    } catch (e) {
      setError(e instanceof Error ? e.message : 'エラー')
    }
  ...
  const canDelete = (u: User) => !u.is_superadmin && u.id !== me?.id

  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">管理者アカウント管理</h1>
      <p className="text-sm text-gray-500 mb-6">
        学生アカウントはクラス管理から作成・削除してください。
      </p>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-base font-semibold mb-4">新規教員アカウント作成</h2>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            {users.map(u => (
              <tr key={u.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 space-x-3">
                  <button
                    onClick={() => setPwChange({ id: u.id, username: u.username, password: '' })}
        ...
        {users.length === 0 && (
          <p className="text-center text-gray-400 py-8">教員アカウントがありません</p>
        )}
      </div>
