...
- 問題の作成・編集（テストケース・サンプルケース・コード制約の設定）
- 問題データの JSON インポート / エクスポート（1問1ファイル）
- 課題の配布（公開開始日時・締切日時を日本時間で指定）
- 課題の編集（タイトル・公開期間の変更）
- 採点結果の一覧表示・CSV エクスポート・提出コード閲覧・ZIP ダウンロード

### 学生
- ブラウザ上での C 言語コード記述・提出（Monaco Editor）
...
| 区分 | 技術 |
|---|---|
| フロントエンド | Next.js 15（App Router）、TypeScript、Tailwind CSS、Monaco Editor |
...
| POST | `/api/v1/submissions` | コード提出・採点 |
